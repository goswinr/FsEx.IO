...
module Kernel32 = 
    ...
    let loadLibrary (path:string) : unit= 
        if IO.File.Exists(path) then  // see https://github.com/dotnet/fsharp/issues/10136
            ...
            FileNotFoundException.Raise "FsEx.IO.Kernel32.loadLibrary cant find file %s" path
    ...
    let freeLibrary (path:string) : unit = 
        ...
        if K32.loadedLibs.ContainsKey(cleanPath) then
            ...
            if not ok then Kernel32Exception.Raise "FsEx.IO.Kernel32.freeLibrary handle is no longer valid. Is the Library already unloaded? %s" cleanPath
        else
            Kernel32Exception.Raise "FsEx.IO.Kernel32.freeLibrary cant find pointer for file to unload, is it not loaded? %s" cleanPath
    ...
    let addDllDirectory (path:string) : unit= 
        if IO.Directory.Exists(path) then
            ...
            DirectoryNotFoundException.Raise "FsEx.IO.Kernel32.addDllDirectory cant find directory %s" path
    ...
    let removeDllDirectory (path:string) : unit = 
        ...
        if K32.loadDirs.ContainsKey(cleanPath) then
            ...
            if not ok then Kernel32Exception.Raise "FsEx.IO.Kernel32.removeDllDirectory handle is no longer valid. Is the Folder already unloaded? %s" cleanPath
        else
            Kernel32Exception.Raise "FsEx.IO.Kernel32.removeDllDirectory cant find pointer for folder to unload, is it not loaded? %s" cleanPath
    ...
    let unblockFile(filePath:string ) : bool = 
        if IO.File.Exists(filePath) then
            ...
            FileNotFoundException.Raise "FsEx.IO.Kernel32.unblock cant find file %s" filePath
